...
df = pd.read_csv(
    output_dir / "selection_ratio_county_2010-2019_bootstraps_1000_poverty.csv",
    dtype={"FIPS": str},
)
sr_df = pd.read_csv(
    output_dir.parent / "misc" / "subregion_counties.csv",
    dtype={"FIPS": str},
...
for state in df.State.unique():
    if (
        len(df[df.State == state].year.unique()) >= 3
        and len(df[df.State == state].FIPS.unique()) > 1
    ):
        df_temp = df_temp.append(df[df.State == state])
df = df_temp


df["sr_and_ci"] = list(zip(df["selection_ratio"], 1 / df["var_log"]))
