...
get_nsduh_data <- function(year) {
  download.file(
    url = glue("https://www.datafiles.samhsa.gov/sites/default/files/field-uploads-protected/studies/NSDUH-{year}/NSDUH-{year}-datasets/NSDUH-{year}-DS0001/NSDUH-{year}-DS0001-bundles-with-study-info/NSDUH-{year}-DS0001-bndl-data-tsv.zip"),
    destfile = here("data_downloading", "R", "nsduh.zip")
  )
  # dir.create(here('data_downloading', 'R', 'nsduh'))
  unzip(here("data_downloading", "R", "nsduh.zip"),
    exdir = here("data_downloading", "R", glue("nsduh_{year}"))
  )
  file.remove(here("data_downloading", "R", "nsduh.zip"))
}

# if filename is already present, just skip everything
filename_to_write <- here("data_downloading", "R", "nsduh", glue('nsduh_usage_{ifelse(aggregate==1, "aggregate_", "")}{min(years)}_{max(years)}{ifelse(hispanics_included == 1, "_hispincluded", "_nohisp")}{ifelse(poverty==1, "_poverty", "")}{ifelse(metro==1, "_metro", "")}.csv'))


# download only files that if they are not present
dir.create(here("data_downloading", "R"))
files_present <- list.files(here("data_downloading", "R"))
...
df_list <- years %>% purrr::map(~ vroom(here(
  "data_downloading", "R",
...
if (hispanics_included == 1) {
  ...
  # we can't really predict race well, so randomizing labels at the moment
...
df_srv <- df %>%
  as_survey_design(
    strata = VESTR, ids = VEREP, weights = ANALWT_C,
    nest = TRUE
  )
...
# generate summary stats for tables ----
...
get_buying_frequency <- function(df4stats_srv) {
  buy <- df4stats_srv %>%
    ...
    mutate(
      value = glue("{value %>% round()} ({value_se %>% round(.)})"),
  ...
  perc_buyers <- df4stats_srv %>%
    ...
    mutate(
      value = glue("{value %>% round(.,2) * 100}% ({round(value_se, 2) * 100})"),
...
get_where_did_you_buy <- function(df4stats_srv) {
  df4stats_srv %>%
    ...
    mutate(
      value = glue("{value %>% round(.,2) * 100}% ({value_se %>% round(.,2) * 100})"),
      value_se = glue("{value_se %>% round(.,2) * 100}%")
    ) %>%
