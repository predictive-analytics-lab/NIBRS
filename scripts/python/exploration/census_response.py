...
sb_df = pd.read_csv(
    data_path
    / "output"
    / "selection_ratio_county_2017-2019_grouped_wilson_poverty.csv",
    dtype={"FIPS": str},
)
...
cr_df = cr_df.rename(
    columns={"Census_Response_Rate_Social_Capital_Proxy": "census_response_all"}
)
cr_df = cr_df.drop(columns=["Name", "cty"])

sb_df = sb_df.merge(cr_df, on=["FIPS"], how="left")

sb_df["cr_county_ratio"] = (
    sb_df["census_response_all"] / sb_df["census_response_all"].mean()
)
...
g = sns.jointplot(
    data=sb_df, x="cr_county_ratio", y="selection_ratio_log10", kind="reg"
)
