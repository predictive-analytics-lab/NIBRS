...
rural_urban = {"Nonmetro": "nonmetro", "Large metro": "metro", "Small metro": "metro"}
...
def reporting_coverage(df: pd.DataFrame, resolution: str) -> pd.DataFrame:
    agency_df = pd.read_csv(
        ...
        usecols=["ori", "nibrs_participated", "population", "data_year"],
    )
    ...
    coverage = (
        (
            agency_df.groupby(resolution).population.sum()
            / df.groupby(resolution).frequency.sum()
        )
        .clip(upper=1)
        .to_frame("coverage")
        .reset_index()
...
def load_and_process_census_data(years: Union[str, List[str]]) -> pd.DataFrame:
    ...
    df = pd.read_csv(
        ...
        dtype={"STATE": object, "COUNTY": object},
    )  # RACES NOT IN COMBINATION
    ...
    df = pd.melt(
        ...
        value_vars=["WA_MALE", "WA_FEMALE", "BA_MALE", "BA_FEMALE"],
        var_name="RACESEX",
...
def add_poverty_info(df: pd.DataFrame):
    poverty_df = pd.read_csv(
        data_path / "misc" / "poverty_data.csv", index_col=False, dtype={"FIPS": str}
    )
    poverty_df = poverty_df.fillna(0)
    poverty_df["FIPS"] = poverty_df.FIPS.apply(lambda x: str(x).rjust(5, "0"))
    poverty_df = poverty_df.rename(
        columns={
            "poverty_income higher than poverty level": "income higher than poverty threshold",
    ...
    df = pd.melt(
        df,
        id_vars=["ori", "FIPS", "year", "state", "state_region", "age", "race", "sex"],
        value_vars=["living in poverty", "income higher than poverty threshold"],
        var_name="poverty",
        value_name="frequency",
    )
    df = df.fillna(0)
...
def add_metro_info(df: pd.DataFrame):
    df_pre_2015 = df[df.year < 2015]
    df_post_2015 = df[df.year >= 2015]

    def _add_metro(df_year: pd.DataFrame, filename_year: str) -> pd.DataFrame:
        if len(df_year) == 0:
            return pd.DataFrame()
        metro_df = pd.read_csv(
            data_path / "misc" / f"urban_codes_x_county_{filename_year}.csv",
            index_col=False,
            dtype={"FIPS": str},
            usecols=["FIPS", "urbancounty"],
        )
        metro_df["FIPS"] = metro_df.FIPS.apply(lambda x: str(x).rjust(5, "0"))
        metro_df["urbancounty"] = metro_df.urbancounty.map(rural_urban)
        metro_df = metro_df.rename(columns={"urbancounty": "metrocounty"})
        return df_year.merge(metro_df, how="left", on="FIPS")

    df_pre_2015 = _add_metro(df_pre_2015, "2003")
    df_post_2015 = _add_metro(df_post_2015, "2013")
    return df_pre_2015.append(df_post_2015)


def get_census_data(years: str, poverty: bool, metro: bool):
    df = load_and_process_census_data(years)
    if poverty:
        df = add_poverty_info(df)
    if metro:
        df = add_metro_info(df)
    return df
