...
def smooth_state(state_df: pd.DataFrame, county_gdf: gpd.GeoDataFrame, state: str,  dem_vars: List[str], value_var: str, urban_filter: int = 2) -> pd.DataFrame:
    ...
    state_df_p = state_df.pivot_table(index=["FIPS"], columns=dem_vars, values=value_var)
    ...
    state_gdf_p = state_gdf_p[["FIPS",  *state_gdf_p.columns[22:].values]].melt(id_vars=["FIPS"], value_name=value_var)
...
def filter_urban(df: pd.DataFrame, urban_level: int, coverage_required: float = 0.5, min_incidents: int = 10) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    coverage = pd.read_csv(data_path / "misc" / "county_coverage.csv", dtype=str, usecols=["FIPS", "coverage", "year"])
    ...
    if "incidents" in df.columns:
        min_df = (df.groupby("FIPS").incidents.sum() >= min_incidents).to_frame("min_threshold").reset_index()
        df = pd.merge(df, min_df, on="FIPS", how="left")
    else:
        df["min_threshold"] = True
    condition = (df.urban_code <= urban_level) & (df.coverage.astype(float) > coverage_required) & (df.min_threshold == True)
