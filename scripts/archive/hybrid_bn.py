...
dag = DAG.from_modelstring(
    "[incident|pop_age:pop_race:pop_sex:region:uses_cannabis][uses_cannabis|pop_age:pop_race:pop_sex][pop_race|pop_age:pop_sex:region][pop_age|pop_sex:region][pop_sex|region][region]")
...
# %%
dag.get_node("uses_cannabis")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("uses_cannabis"))
dag.get_node("uses_cannabis")["CPD"].parents = [
    'pop_age', 'pop_race', 'pop_sex']
...
dag.get_node("pop_race")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("pop_race"))
dag.get_node("pop_race")["CPD"].array = get_cpd(
    df, "RACE", ["AGE", "SEX", "STATEREGION"])
dag.get_node("pop_race")["CPD"].rescale_probabilities()

dag.get_node("pop_sex")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("pop_sex"))
dag.get_node("pop_sex")["CPD"].array = get_cpd(df, "SEX", ["STATEREGION"])
dag.get_node("pop_sex")["CPD"].rescale_probabilities()

dag.get_node("pop_age")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("pop_age"))
dag.get_node("pop_age")["CPD"].array = get_cpd(
    df, "AGE", ["SEX", "STATEREGION"])
dag.get_node("pop_age")["CPD"].rescale_probabilities()

dag.get_node("region")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("region"))
...
fips_ori = pd.read_csv(data_path / "misc" / "LEAIC.tsv", delimiter="\t",
...
def get_incident_cpt(pop_df, usage_df, inc_df, dem_order):
    cross = pd.crosstab(usage_df.MJDAY30A, [
                        usage_df[col] for col in dem_order], normalize="columns")
    cross_mult = cross.multiply(cross.index, axis="rows")
    cross_sum = cross_mult.sum(axis="rows") / 30
    # pop_df = pop_df[pop_df["FIPS"].isin(inc_df.FIPS.unique().tolist())]
    grouped_incidents = inc_df.groupby(cols + ["STATEREGION"]).size()
    expected_incidents = (pop_df.groupby(
        cols + ["STATEREGION"])["frequency"].sum() * cross_sum * 365)
...
dag.get_node("incident")["CPD"] = baynet.parameters.ConditionalProbabilityTable(
    dag.get_node("incident"))
dag.get_node("incident")["CPD"].array = inc_cpt
dag.get_node("incident")["CPD"].parents = [
    'uses_cannabis', 'pop_age', 'pop_race', 'pop_sex', 'region']
...
def get_selection_by_vars(bn: DAG, county_name: str, race_level: str):
    ...
    local_bn.get_node("pop_race")["CPD"].array = _set_cpt(
        "pop_race", race_level)
...
g = sns.FacetGrid(plotting_data, row='SEX',  col='AGE', hue='TYPE', sharex=False,
                  palette=p, height=5, aspect=5/5, gridspec_kws={"hspace": 0.4, "wspace": 0})
